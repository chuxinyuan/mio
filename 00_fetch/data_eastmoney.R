...
get_json = function(url, query = list(), n = 3L, timeout_s = 20) {
  ...
  last = NULL
  for (i in seq_len(n)) {
    res = tryCatch(GET(url, query = query, timeout(timeout_s),
...
fetch_sse50 = function() {
  parsed = get_json("http://push2.eastmoney.com/api/qt/clist/get",
                    query = list(pn = 1, pz = 200, po = 1, np = 1,
...
fetch_daily = function(code, to = Sys.Date()) {
  parsed = get_json("http://push2his.eastmoney.com/api/qt/stock/kline/get",
                    query = list(
                      secid   = secid(code),
  ...
  if (is.null(klines) || length(klines) == 0) {
    return(data.table(date = character(), open = numeric(), high = numeric(),
...
refresh_universe = function(con = NULL, to = Sys.Date(),
                           throttle_s = 1, verbose = TRUE) {
  own = is.null(con)
  ...
  for (i in seq_len(nrow(sse))) {
    code = sse$code[i]
    d = tryCatch(fetch_daily(code, to = to), error = function(e) NULL)
    if (is.null(d) || nrow(d) == 0) {
      if (verbose) cat(sprintf("[%02d/%02d] %s 失败\n", i, nrow(sse), code))
      next
    }
    replace_bars(con, code, d)
    if (verbose) cat(sprintf("[%02d/%02d] %s %s %d 行\n", i, nrow(sse), code,
                             sse$name[i], nrow(d)))
