...
init_db = function(con = NULL) {
  ...
  dbExecute(con, "CREATE TABLE IF NOT EXISTS symbol (
  ...
  dbExecute(con, "CREATE TABLE IF NOT EXISTS daily_bar (
  ...
  dbExecute(con, "CREATE TABLE IF NOT EXISTS order_ticket (
  ...
  dbExecute(con, "CREATE TABLE IF NOT EXISTS fill (
    id       INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  dbExecute(con, "CREATE TABLE IF NOT EXISTS position (
  ...
  dbExecute(con, "CREATE TABLE IF NOT EXISTS meta (
    key   TEXT PRIMARY KEY,
    value TEXT
  );")

  dbExecute(con, "CREATE TABLE IF NOT EXISTS settings (
    key   TEXT PRIMARY KEY,
    value TEXT
  );")
  ...
  dbExecute(con, "CREATE TABLE IF NOT EXISTS account_snapshot (
    ts     TEXT NOT NULL,
    cash   REAL NOT NULL,
    equity REAL NOT NULL
  );")

  dbExecute(con, "CREATE TABLE IF NOT EXISTS sys_log (
    ts      TEXT NOT NULL,
    level   TEXT NOT NULL,
...
upsert_symbols = function(con, dt) {
  dbExecute(con, "INSERT INTO symbol (code, name, board, is_valid)
...
upsert_bars = function(con, dt) {
  dbExecute(con, "INSERT INTO daily_bar (symbol, date, open, high, low, close, volume)
...
replace_bars = function(con, symbol, dt) {
  dbExecute(con, "DELETE FROM daily_bar WHERE symbol = :symbol",
            list(symbol = symbol))
...
load_bars = function(con, symbols = NULL, from = NULL, to = NULL) {
  ...
  if (!is.null(from)) { cond = c(cond, "date >= ?"); params = c(params, as.character(from)) }
  if (!is.null(to))   { cond = c(cond, "date <= ?"); params = c(params, as.character(to)) }
...
last_date = function(con, symbol) {
  r = dbGetQuery(con, "SELECT MAX(date) AS d FROM daily_bar WHERE symbol = :symbol",
                 list(symbol = symbol))
  r$d[[1]]
}

last_close = function(con, symbol) {
  r = dbGetQuery(con, "SELECT close FROM daily_bar WHERE symbol = :symbol
...
save_order = function(con, ts, symbol, side, qty, price = NA_real_) {
  dbExecute(con, "INSERT INTO order_ticket (ts, symbol, side, qty, price, status)
...
set_order_status = function(con, id, status) {
  dbExecute(con, "UPDATE order_ticket SET status = :status WHERE id = :id",
            list(id = id, status = status))
  invisible(id)
}

save_fill = function(con, order_id, ts, price, qty) {
  dbExecute(con, "INSERT INTO fill (order_id, ts, price, qty)
...
get_orders = function(con, status = NULL) {
  ...
  as.data.table(dbGetQuery(con, q, params = if (is.null(status)) NULL else list(status = status)))
}
...
upsert_position = function(con, symbol, qty, avail_qty, avg_cost) {
  dbExecute(con, "INSERT INTO position (symbol, qty, avail_qty, avg_cost)
...
set_meta = function(con, key, value) {
  dbExecute(con, "INSERT INTO meta (key, value) VALUES (:key, :value)
...
set_setting = function(con, key, value) {
  dbExecute(con, "INSERT INTO settings (key, value) VALUES (:key, :value)
                 ON CONFLICT(key) DO UPDATE SET value = :value",
            list(key = key, value = as.character(value)))
  invisible(value)
}
...
save_snapshot = function(con, ts, cash, equity) {
  dbExecute(con, "INSERT INTO account_snapshot (ts, cash, equity) VALUES (:ts, :cash, :equity)",
...
# ---- log ----
write_log = function(con, message, level = "info", module = "system", ts = format(Sys.time(), "%Y-%m-%d %H:%M:%S")) {
  dbExecute(con, "INSERT INTO sys_log (ts, level, module, message) VALUES (:ts, :level, :module, :message)",
            list(ts = ts, level = level, module = module, message = message))
  invisible(TRUE)
}

get_logs = function(con, limit = 500) {
  as.data.table(dbGetQuery(con, paste0("SELECT * FROM sys_log ORDER BY ts DESC LIMIT ", as.integer(limit))))
}
