...
strategy_server = function(id, con, rv) {
  moduleServer(id, function(input, output, session) {
    ...
    params = reactive({
      list(param = c(n1 = input$n1, n_fact = input$n2 / input$n1,
                     n_sharpe = input$n_sharpe, sh_thresh = input$sh_thresh),
           year = input$year)
    })
    ...
    bt = eventReactive(input$run_bt, {
      req(!is.null(rv$data))
      p = params()
      res = evaluate(rv$data, p$param, year = p$year, settings = rv$settings,
                     transform = FALSE, return_data = TRUE)
      list(equity = res$equity, dates = index(rv$data$close), param = p$param)
    })
    ...
    observeEvent(input$optimize, {
      ...
      best = tryCatch(optimize_params(rv$data, year = input$year),
                      error = function(e) { showNotification(paste("优化失败:", e$message), type = "error"); NULL })
      req(!is.null(best))
    ...
    observeEvent(input$refresh, {
      ...
      sse = tryCatch(refresh_universe(con, verbose = FALSE),
                     error = function(e) { showNotification(paste("刷新失败:", e$message), type = "error"); NULL })
      reload()
      showNotification(if (is.null(sse)) "刷新失败" else paste("刷新完成", nrow(sse), "只"),
    ...
    observeEvent(input$auto_trade, {
      ...
      res = tryCatch(
        auto_trade(con, rv$data, input$n1, input$n2, input$n_sharpe, input$sh_thresh,
                   rv$settings$max_assets, price_map),
        error = function(e) { showNotification(paste("自动下单失败:", e$message), type = "error"); NULL })
      if (!is.null(res)) {
        n_ok = sum(vapply(res, function(r) isTRUE(r$ok), logical(1)))
        showNotification(paste0("已按信号下单（成功 ", n_ok, " / ", length(res), " 笔）"),
    ...
    output$signals = renderDT({
      req(!is.null(rv$data))
      sig = latest_signals(rv$data, input$n1, input$n2, input$n_sharpe, input$sh_thresh)
