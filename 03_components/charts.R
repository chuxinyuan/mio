...
kline_chart = function(d) {
  d |>
    e_charts(date) |>
    e_candle(open, close, low, high, name = "K线",
             itemStyle = list(color = RED_UP, color0 = GREEN_DOWN,
                              borderColor = RED_UP, borderColor0 = GREEN_DOWN)) |>
    ...
    e_bar(volume, name = "成交量", y_index = 1,
          itemStyle = list(color = "rgba(0,240,255,0.35)")) |>
    e_grid(height = c("56%", "22%")) |>
    e_x_axis(type = "category", axisLabel = list(color = AXIS_TEXT),
             axisLine = list(lineStyle = list(color = "rgba(0,240,255,0.3)"))) |>
    e_y_axis(index = 0, scale = TRUE, axisLabel = list(color = AXIS_TEXT),
             splitLine = list(lineStyle = list(color = "rgba(0,240,255,0.08)"))) |>
    e_y_axis(index = 1, scale = TRUE, axisLabel = list(show = FALSE),
...
equity_chart = function(d) {
  d |>
    e_charts(ts) |>
    e_line(equity, name = "权益", lineStyle = list(width = 2, color = CYAN,
                                                   shadowColor = "rgba(0,240,255,0.6)", shadowBlur = 10)) |>
    e_area(equity, name = "权益", lineStyle = list(width = 2, color = CYAN),
           itemStyle = list(color = "rgba(0,240,255,0.20)")) |>
    e_x_axis(axisLabel = list(color = AXIS_TEXT),
             axisLine = list(lineStyle = list(color = "rgba(0,240,255,0.3)"))) |>
    e_y_axis(scale = TRUE, axisLabel = list(color = AXIS_TEXT),
             splitLine = list(lineStyle = list(color = "rgba(0,240,255,0.08)"))) |>
